 ...
 *******************************************************************************/
package com.baldapps.artemis.utils.flags;
...
public abstract class VariableFlags {
	protected static final int READ = PDOMName.READ_ACCESS;
	protected static final int WRITE = PDOMName.WRITE_ACCESS;

	public static Optional<Integer> getFlags(IASTName name) {
		if (name.getTranslationUnit().getLinkage().getLinkageID() == ILinkage.CPP_LINKAGE_ID) {
			return CPPVariableFlags.getReadWriteFlags(name);
		} else {
			return CVariableFlags.getReadWriteFlags(name);
		}
	}
